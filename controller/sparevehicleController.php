...
Class sparevehicleController Extends baseController {
    ...
    public function index() {
        ...
        $spare_parts = $spare_part_model->getAllStock(array('order_by'=>'spare_part_name ASC, spare_part_code','order'=>'ASC'));
        $this->view->data['spare_parts'] = $spare_parts;

        $sparevehicle_model = $this->model->get('sparevehicleModel');

        $join = array('table'=>'vehicle,spare_part','where'=>'vehicle = vehicle_id AND spare_part_id');
        $data = array(
        ...
        $join = array('table'=>'romooc,spare_part','where'=>'romooc = romooc_id AND spare_part_id');
        $data = array(
            'where' => '((end_time IS NULL OR end_time = 0) OR end_time >= '.strtotime(date('d-m-Y')).')',
        );
    ...
    public function exchange(){
        ...
        if (isset($_POST['yes'])) {
            ...
            $dm1 = $sparevehicle->queryStock('SELECT * FROM spare_vehicle WHERE romooc='.$data['romooc'].' AND start_time <= '.$data['start_time'].' AND (end_time IS NULL OR end_time > '.$data['start_time'].') ORDER BY start_time DESC LIMIT 1');
            $dm2 = $sparevehicle->queryStock('SELECT * FROM spare_vehicle WHERE romooc='.$data['romooc'].' AND start_time > '.$data['start_time'].' AND (end_time IS NULL OR end_time > '.$data['start_time'].') ORDER BY start_time ASC LIMIT 1');
            $dm3 = $sparevehicle->queryStock('SELECT * FROM spare_vehicle WHERE vehicle='.$data['vehicle'].' AND start_time <= '.$data['start_time'].' AND (end_time IS NULL OR end_time > '.$data['start_time'].') ORDER BY start_time DESC LIMIT 1');
            $dm4 = $sparevehicle->queryStock('SELECT * FROM spare_vehicle WHERE vehicle='.$data['vehicle'].' AND start_time > '.$data['start_time'].' AND (end_time IS NULL OR end_time > '.$data['start_time'].') ORDER BY start_time ASC LIMIT 1');
            ...
            if ($dm1 || $dm2 || $dm3 || $dm4) {
                if($dm1){
                    foreach ($dm1 as $row) {
                        ...
                        $sparevehicle->updateStock($d,array('spare_vehicle_id'=>$row->spare_vehicle_id));
                    }
                }
                else if ($dm2) {
                    foreach ($dm2 as $row) {
